...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /*
   * NOTE DÉVELOPPEUR — INTÉGRATION FUTURE EXPRESS + POSTGRES
   * =========================================================
   * Quand tu auras ton API Express prête, tu devras :
   *
   * 1. Créer un endpoint GET /api/categories dans Express :
   *    app.get('/api/categories', async (req, res) => {
   *      const result = await pool.query('SELECT * FROM categories ORDER BY sort_order');
   *      res.json(result.rows);
   *    });
   *
   * 2. Dans loadCategories() ici, remplacer le return MOCK_CATEGORIES par :
   *    const res = await fetch('/api/categories');
   *    return res.json();
   *
   * 3. Décommenter l'appel _renderFromData() dans _init() pour
   *    que le menu se génère dynamiquement depuis la base de données.
   *
   * Structure table PostgreSQL suggérée :
   ...
   *     id       UUID PRIMARY KEY DEFAULT gen_random_uuid(),
   *     group_id UUID REFERENCES subcategory_groups(id),
   *     label    VARCHAR(100),
